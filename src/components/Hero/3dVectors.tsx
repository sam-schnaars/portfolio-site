...
const VectorInputForm = ({ addVector, updateVector, deleteVector, vectors, selectedIndex, setSelectedIndex, dotProductTarget, setDotProductTarget, isOpen, toggleForm }) => {
  ...
  return (
    ...
    >
      <div className="bg-gray-800 p-3 rounded-lg text-white w-64 shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col space-y-2">
          <div className="flex flex-col space-y-2">
            ...
            ].map(([label, val, setFunc]: [string, number, (value: number) => void], i) => (
...
const VectorLegend = ({ vectors }) => {
  return (
    <div className="absolute top-2 right-2 bg-black bg-opacity-50 p-1 rounded text-white text-xs">
      {vectors.map((vector, idx) => (
...
const Spinning3DObject = ({ showForm = true }) => {
  ...
  const [dotProductTarget, setDotProductTarget] = useState(null);
  ...
  const deleteVector = () => {
    ...
    } else if (dotProductTarget !== null && dotProductTarget > selectedIndex) {
